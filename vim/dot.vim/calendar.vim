...
function! Cream_calendar()
	" close
	if exists("g:CREAM_CALENDAR")
		" leave buffer if in calendar
		...
		let mybufnr = bufnr("%")
		" close
		if bufexists(bufname("__Calendar")) > 0
			execute "bwipeout! __Calendar"
		endif
	...
	" open
		...
		" start with Monday
		let g:calendar_monday = 1
		" show week number (only if monday off)
		"let g:cream_calendar_weeknm = 1
		" current day indication
		let g:calendar_mark = 'left'
		" navigator
		let g:calendar_navi = 'top'
		"" holiday file
		"let g:calendar_diary = $CREAM

		" call command, not function!
		execute ":Calendar"
		let g:CREAM_CALENDAR = "1"
		" reset window configuration
		call Cream_window_setup()
		" restore cursor to original current buffer's new window
		call MoveCursorToWindow(bufwinnr(mybufnr))
...
function! Cream_calendar_init()
" initialize calendar if it was open at last exit
	if exists("g:CREAM_CALENDAR")
		" reverse state
		unlet g:CREAM_CALENDAR
		call Cream_calendar()
		" go to another buffer if current
		if bufname("%") == "__Calendar"
			call Cream_nextwindow()
		endif
		" for some reason (autocmd?) we end up in normal mode
		startinsert
	endif
endfunction

function! Cream_calendar_exit()
" used by Cream_exit() to close calendar and remember state before exiting
	" go to another buffer if current
	if bufname("%") == "__Calendar"
		call Cream_nextwindow()
	endif
...
""*** Too slow!
"" holidays
"" Source: http://vim.sourceforge.net/tip_view.php?tip_id=408
"let calendar_sign = 'MyGetSpecialDay'
"function! MyGetSpecialDay(day, month, year)
"    let m100d = 10000 + (a:month * 100 ) + a:day
"    let holidays = $CREAM . "calendar-holidays_US"
"    execute "silent! split " . holidays
"    let found = search(m100d)
"    if found
"        let found = 'h'
"    endif
"    silent! quit
...
if !exists("g:calendar_diary")
  let g:calendar_diary = "~/Sync/diary"
endif
...
function! Calendar(...)
  ...
  "+++ Cream: need insert mode mapping
  execute 'imap <silent> <buffer> <cr> <C-b>:call <SID>CalendarDoAction()<cr>'
  execute 'imap <silent> <buffer> <2-LeftMouse> <C-b>:call <SID>CalendarDoAction()<cr>'
  "+++
  "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  "+++ build highlight
  "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
